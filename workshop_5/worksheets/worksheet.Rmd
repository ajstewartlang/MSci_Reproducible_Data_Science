...
2. Using either a built in dataset (e.g., the `mpg` or `starwars` dataset) or an external one (e.g., the `NHANES` dataset which is accessible in the package called `NHANES`) build some data visualisations to communicate some interesting aspect of the data. You might like to play around with the `gganimate` and `ggforce` packages.
